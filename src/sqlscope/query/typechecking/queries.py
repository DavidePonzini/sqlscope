...
@get_type.register
def _(expression: exp.Select, catalog: Catalog, search_path: str, dialect: Dialect | None = None) -> ResultType:
    types = []
    old_messages = []

    for col in expression.expressions:
        col_type = get_type(col, catalog, search_path, dialect)
    ...
    if where:
        old_messages.extend(get_type(where.this, catalog, search_path, dialect).messages)

    having = expression.args.get("having")
    if having:
        old_messages.extend(get_type(having.this, catalog, search_path, dialect).messages)
    if len(types) == 1:
...
@get_type.register
def _(expression: exp.Subquery, catalog: Catalog, search_path: str, dialect: Dialect | None = None) -> ResultType:
